 ...
 */
function cohort_get_invisible_contexts_manager() {
    global $DB;
    if (is_siteadmin()) {
 ...
 */
function cohort_edit_controls_manager(context $context, moodle_url $currenturl) {
    $tabs = array();
    $currenttab = 'view';
    $viewurl = new moodle_url('/phlcohort/manager/index.php', array('contextid' => $context->id));
    ...
    if (has_capability('moodle/cohort:manage', $context)) {
        $addurl = new moodle_url('/phlcohort/manager/edit.php', array('contextid' => $context->id));
        ...
        //$uploadurl = new moodle_url('/phlcohort/manager/upload.php', array('contextid' => $context->id));
 ...
 */
function core_cohort_inplace_editable_manager($itemtype, $itemid, $newvalue) {
...
and q.course = c.id
");
    $listCourse='<select width: 550% id="sldownloadketqua'.$cohortID.'" onchange="download_ketquahoc('.$cohortID.')"><option value="" >================ Tai Ket Qua Khoa Hoc ================</option>';
    foreach ($listRecords as $record) {
        ...
        //       $get_course_id = "select id from {course} where fullname = '$record->fullname'";
        
        //     $course = $DB->get_record_sql($get_course_id,array());
        //     foreach($course as $c){
        //        $saved_course = $c->id;
        //    }
        
        //   $myfile = fopen(__DIR__ ."/../report/completion/fullname.txt", "w") or die("Unable to open file!");
        //    fwrite($myfile,$record->fullname);
        //    fclose($myfile);
    }
    $listCourse.='</select>';
    return $listCourse;
